...
**Profile** — exact WoW or subsystem target identified by flavor, Interface/build, source revision/digest, schema/tool versions and correction/configuration set. `Current` is not a durable profile ID.

**Reference generation** — immutable identity of one built Reference Pack.

**Project generation** — immutable project snapshot published from coherent source/configuration inputs against one exact profile.
...
**ResynchronizationRequired** — frontend document state where a stale, skipped, repeated-with-different-content or out-of-order change cannot be applied. The client must send a new full exact document state; no suspect partial change is retained.
...
**ContextSemanticPack** — immutable exact owner-evidence context artifact built from one coherent `ContextUniverseSet`; rendered or external sidecar text is not part of its truth.
...
**External locator mapping request candidate** — E6-A handoff containing exact candidate/locator fields for E6-B owner mapping; it is not a source handle.
...
**ExternalProviderUseAuthorization** — nonsecret receipt authorizing use of one exact provider configuration/reference for one operation/query purpose. It does not contain secret material or create semantic authority.
...
**NoMappingWithOwnerAuthority** — exact owner proved complete relevant coverage and no matching record. It does not prove the provider concept or broader source is absent.
...
**Exact mapped root** — stable project/reference owner handle from an `ExactMapped` record, used unchanged as the root of an existing E3 context operation.
...
**FrontendProtocolProfile** — exact immutable profile for one CLI/daemon/LSP/MCP transport, including wire revision, framing, limits, lifecycle, security, position encodings and operation-registry identity.

**FrontendOperationDescriptor** — exact binding from one wire command/method/tool/resource operation to one service request/result/error schema, owner capabilities, effect/authorization class, privacy/license, budgets, cancellation/progress and nonclaims.

**FrontendOperationRegistry** — immutable deterministic set of reviewed frontend operation descriptors and implementation availability. It is the only source of advertised operations; runtime reflection cannot widen it.

**FrontendCapabilitySet** — exact negotiated intersection of registry, implementation, client protocol capabilities, workspace/profile state and consumer/privacy/license policy. Negotiation only narrows.

**FrontendClientIdentity** — transport-scoped client implementation/protocol metadata. It is compatibility/provenance input and never semantic or effect authorization.

**FrontendSession** — exact protocol/client/registry/consumer scope containing workspace registrations, overlay heads, operation tickets, leases, response journals and close state.

**FrontendWorkspaceRegistration** — exact session-scoped explicit root/path and project/profile/access/privacy/license registration. A root is untrusted input and not proof of a valid addon/project.

**FrontendDocumentOverlay** — immutable session-scoped unsaved document snapshot binding URI, workspace, client version, UTF-8 bytes/digest, line index, exact prior overlay, persisted base and privacy state.

**OverlayAnalyzerSnapshot** — exact EmmyLua analysis result over a project-owned overlay input, with explicit overlay-local versus published-dependency freshness and canonical UTF-8 ranges.

**FrontendOperationTicket** — exact transport request/session/registry/service request/workspace/document/overlay binding and optional durable operation identity, progress/cancellation/result/delivery state.

**FrontendProgressEvent** — bounded nonauthoritative operational stage/counter notification. It cannot establish completion, coverage or effect state.

**FrontendResponseJournalEntry** — exact record of final response preparation/delivery/replay eligibility. It records delivery only and replay never reexecutes service.

**Position encoding** — session-fixed LSP character encoding (`utf-16` or supported `utf-8`) used only to project positions against an exact overlay line index. Owner source ranges remain UTF-8 byte offsets.

**Local daemon profile** — `wow-local-jsonrpc/1` over current-user Windows named pipe or Unix-domain socket. It is foreground/local by default and not a remote service.

**LSP 3.18 profile** — exact stdio projection of implemented lifecycle, workspace/document, diagnostics and language operations under Language Server Protocol 3.18.

**MCP 2025-11-25 profile** — exact Model Context Protocol revision with fixed read-only tools/resources over stdio by default. Prompts, sampling, elicitation, tasks, arbitrary roots and effecting default tools are absent.

**Exact `wow://` resource URI** — consumer-scoped URI containing one exact retained profile/generation/result/artifact/publication identity; never floating `current/latest/best` or a raw filesystem/provider path.

**Disconnect** — transport loss only. It is not cancellation and does not prove no owner effect.

**Response replay** — redelivery of exact retained final bytes to an authorized session/consumer. It never invokes the service operation again.

**Backpressure policy** — bounded per-session/connection queues where final results/errors/state transitions outrank progress/logs; dropped/coalesced low-priority events remain explicit.
...
**Universe** — separated source domain such as Reference Pack, first-party project, Blizzard UI source, calibration corpus, external candidate, overlay, runtime or history.