...
impl GeneratedApiFact {
    ...
    pub fn restrictions(&self) -> Option<&Map<String, Value>> {
        self.normalized.get("restrictions").and_then(Value::as_object)
    }
}
...
fn parse_coverage(value: &Value) -> GeneratedApiImportResult<GeneratedApiCoverage> {
    ...
    let expected_collections = GeneratedApiFactKind::all()
        ...
        .collect::<BTreeSet<_>>();
    if count_map.keys().map(String::as_str).collect::<BTreeSet<_>>() != expected_collections {
        return Err(error(
            GeneratedApiImportErrorCode::InvalidCoverage,
...
fn parse_fact(
    ...
    for collection in ["arguments", "returns", "payload", "fields", "values"] {
        let records = array(required(member, collection, collection)?, collection)?;
...
fn parse_conflicts(
    ...
    for value in values {
        ...
        let collection =
            nonempty_string(conflict, "collection", "conflict collection")?.to_owned();
        let qualified_name =
        ...
        sources.sort_by(|left, right| {
            (left.path.as_str(), left.line_start)
                .cmp(&(right.path.as_str(), right.line_start))
        });
...
mod tests {
    ...
    fn seal(mut value: Value) -> GeneratedApiImportResult<Vec<u8>> {
        ...
        object_mut(&mut value, "test draft")?.insert(
            "draft_sha256".to_owned(),
    ...
    #[test]
    fn complete_import_supports_exact_positive_and_negative_lookup(
    ) -> GeneratedApiImportResult<()> {
