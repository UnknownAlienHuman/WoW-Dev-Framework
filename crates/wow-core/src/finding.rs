...
struct FindingIdentity<'a> {
    ...
    coverage_identity: Vec<CoverageIdentity<'a>>,
...
struct CoverageIdentity<'a> {
    ...
    partition: &'a crate::CoveragePartitionId,
    status: CoverageStatus,
    producer: &'a crate::ProducerId,
}
...
fn validate_input(input: &FindingInput) -> CoreResult<()> {
    ...
    if input.remediation == Some(RemediationClass::ExactEdit) {
        ...
        let complete_coverage = input.coverage.iter().all(|record| {
            matches!(
                record.status(),
...
fn canonical_finding_key_from_input(input: &FindingInput) -> CoreResult<FindingKey> {
    let evidence_ids = input
        ...
        .collect::<CoreResult<Vec<_>>>()?;
    let coverage_identity = input
        .coverage
        .iter()
        .map(|record| CoverageIdentity {
...
impl<'de> Deserialize<'de> for Finding {
        ...
        D: Deserializer<'de>,
    {
        ...
        let finding = Finding::new(FindingInput {
            ...
            message_arguments: wire.message_arguments,
            primary_source: wire.primary_source,
            related_sources: wire.related_sources,
