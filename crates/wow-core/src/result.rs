    ...
    pub fn canonicalize(&mut self) -> CoreResult<()> {
        ...
        ) && self.reasons.is_empty()
        {
            return Err(CoreError::new(
                ...
                "incomplete capability state requires a reason",
            ));
        }
        Ok(())
    }
...
pub fn canonical_result_order(envelope: &mut ResultEnvelope) -> CoreResult<()> {
    ...
    let mut unique_findings: Vec<Finding> = Vec::with_capacity(envelope.findings.len());
    ...
    for capability in &envelope.capabilities {
        ...
        if canonical != *capability {
            return Err(CoreError::new(
                CoreErrorCode::ResultContextViolation,
                "validate_result_envelope",
                format!("capability state {} is not canonical", capability.capability),
            ));
        }
    ...
    for record in &envelope.not_evaluated {
        ...
        if canonical != *record {
            return Err(CoreError::new(
                CoreErrorCode::ResultContextViolation,
                "validate_result_envelope",
                format!("NotEvaluated record {} is not canonical", record.subject),
            ));
        }
