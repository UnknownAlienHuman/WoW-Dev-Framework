...
fn transaction_order_single_wrapper_and_sequential_results_agree() -> TestResult {
    ...
    assert_eq!(
        serde_json::to_vec(forward.candidate())?,
        serde_json::to_vec(&sequential)?
    );
    ...
    let restored: GraphPartitionSnapshot =
        serde_json::from_slice(&serde_json::to_vec(after.as_ref())?)?;
...
fn transaction_removes_dependent_owners_together_and_preserves_survivors() -> TestResult {
    ...
    let saved_c = old
        .partition("producer:c")
        .ok_or("surviving partition")?
        .clone();
    ...
    for change in plan.changes() {
        assert_eq!(
            change.previous_partition_digest(),
            Some(
                old.partition(change.partition_id())
                    .ok_or("old partition")?
                    .partition_digest()
            )
        );
...
fn transaction_checks_all_base_and_partition_guards_before_publication() -> TestResult {
    ...
    for guard in 0..3 {
        ...
        assert_eq!(
            before
                .prepare_replacements(vec![valid, stale], &stop)
                .err()
                .ok_or("stale transaction accepted")?
                .code(),
    ...
    assert_eq!(
        before
            .prepare_replacements(vec![duplicate.clone(), duplicate], &stop)
            .err()
...
fn rejected_peer_and_removed_peer_endpoint_abort_whole_transaction() -> TestResult {
    ...
    for requests in [vec![a.clone(), b.clone()], vec![b, a]] {
        assert_eq!(
            before
                .prepare_replacements(requests, &stop)
                .err()
                .ok_or("peer endpoint accepted")?
                .code(),
    ...
    assert_eq!(
        before
            .prepare_replacements(
                vec![
                    request(&before, batch("producer:a", Some("NewA"))?)?,
                    elevated,
                ],
                &stop
            )
            .err()
            .ok_or("elevated peer accepted")?
            .code(),
        GraphErrorCode::PartitionInvalid
    );
    assert!(Arc::ptr_eq(&before, &session.view()));
...
fn transaction_cancellation_stale_plan_and_no_change_preserve_arc_identity() -> TestResult {
    ...
    assert_eq!(
        before
            .prepare_replacements(requests.clone(), &AtomicBool::new(true))
            .err()
            .ok_or("cancelled transaction accepted")?
            .code(),
    ...
    assert_eq!(
        session
            .publish(plan.clone(), &AtomicBool::new(true))
            .err()
    ...
    assert_eq!(
        session
            .publish(competing, &stop)
            .err()
            .ok_or("stale plan accepted")?
            .code(),
        GraphErrorCode::PartitionStale
    );
    let repeat = active.prepare_replacements(
        vec![
    ...
    assert!(
        repeat
            .changes()
            .iter()
            .all(|change| change.previous_partition_digest()
                == Some(change.target_partition_digest()))
    );
...
fn transaction_cardinality_and_combined_assertion_limits_are_enforced() -> TestResult {
    ...
    let base = initial()?;
    assert_eq!(
        base.prepare_replacements(Vec::new(), &stop)
            .err()
            .ok_or("empty transaction accepted")?
            .code(),
        GraphErrorCode::PartitionInvalid
    );
    let request_a = request(&base, batch("producer:a", None)?)?;
    assert_eq!(
        base.prepare_replacements(vec![request_a; 65], &stop)
            .err()
            .ok_or("oversized transaction accepted")?
            .code(),
    ...
    for index in 0..64 {
        requests.push(request(
            &base,
            batch(&format!("producer:{index:02}"), None)?,
        )?);
    }
    let full = base
        .prepare_replacements(requests, &stop)?
    ...
    assert_eq!(
        full.prepare_replacements(vec![request(&full, batch("producer:new", None)?)?], &stop)
            .err()
            .ok_or("65th partition accepted")?
            .code(),
        GraphErrorCode::BudgetExceeded
    );
    ...
    let small = GraphSnapshot::build(
        universe()?,
        generation()?,
        GraphLimits::new(2, 10, 10, 10, 10)?,
        base.foundation().nodes().to_vec(),
        Vec::new(),
        coverage()?,
    )?;
    ...
    assert_eq!(
        limited
            .prepare_replacements(vec![a, b], &stop)
            .err()
            .ok_or("combined budget exceeded")?
            .code(),
        GraphErrorCode::BudgetExceeded
    );
    Ok(())
