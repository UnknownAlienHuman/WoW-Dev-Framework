...
fn registry() -> TestResult<GraphRegistryBundle> {
    Ok(GraphRegistryBundle::build(
        "graph-registry:partition-fixture",
        "1.0.0",
...
fn generation() -> TestResult<GraphGenerationId> {
    Ok(GraphGenerationId::new(
        "project-generation:partition-fixture:1",
    )?)
}
...
fn batch(id: &str, symbol: Option<&str>) -> TestResult<GraphProposalBatch> {
    ...
    let entities = if let Some(symbol) = symbol {
        vec![GraphEntityProposal::new(
            ...
            BTreeMap::from([(
                Box::<str>::from("symbol"),
                GraphProposalValue::Identifier(symbol.into()),
            )]),
...
fn edge_batch(id: &str, from: &GraphNodeId, to: &GraphNodeId) -> TestResult<GraphProposalBatch> {
    ...
    let edge = GraphRelationProposal::new(
        ...
        GraphRelationProposalInput {
            ...
            evidence_ids: vec![EvidenceId::derive(&(id, "edge"))?],
...
fn replace(
    ...
    Ok(view
        .prepare_replacement(request(view, batch)?, &AtomicBool::new(false))?
        .candidate()
        .clone())
}
...
fn contributed_node(view: &GraphPartitionSnapshot) -> TestResult<GraphNode> {
    Ok(view
        .input_view(&AtomicBool::new(false))?
        .nodes()
        .iter()
        .find(|node| node.owner_key() != "source:root")
        .ok_or("contributed node missing")?
        .clone())
}
...
fn independent_partition_order_has_identical_generation_and_bytes() -> TestResult {
    let initial = initial()?;
    let left = replace(
        &replace(&initial, batch("producer:a", Some("A"))?)?,
        batch("producer:b", Some("B"))?,
    )?;
    let right = replace(
        &replace(&initial, batch("producer:b", Some("B"))?)?,
        batch("producer:a", Some("A"))?,
    )?;
    assert_eq!(left, right);
    assert_eq!(serde_json::to_vec(&left)?, serde_json::to_vec(&right)?);
    assert_ne!(
        left.snapshot().generation(),
        initial.snapshot().generation()
    );
    assert_eq!(
        left.input_view(&AtomicBool::new(false))?.generation(),
        &generation()?
    );
    assert!(
        left.snapshot()
            .generation()
            .as_str()
            .starts_with("graph-generation:sha256:")
    );
    assert!(
        left.snapshot()
            .nodes()
            .iter()
            .all(|node| node.generation() == left.snapshot().generation())
    );
...
fn replacement_removes_stale_nodes_and_preserves_old_readers() -> TestResult {
    ...
    let plan =
        old.prepare_replacement(request(&old, batch("producer:a", Some("New"))?)?, &stopped)?;
    ...
    let repeat = current.prepare_replacement(
        request(&current, batch("producer:a", Some("New"))?)?,
        &stopped,
    )?;
...
fn stale_snapshot_partition_version_and_competing_plan_do_not_publish() -> TestResult {
    ...
    let plan_a =
        first.prepare_replacement(request(&first, batch("producer:a", Some("A"))?)?, &stopped)?;
    let plan_b =
        first.prepare_replacement(request(&first, batch("producer:b", Some("B"))?)?, &stopped)?;
    let active = session.publish(plan_a, &stopped)?;
    assert_eq!(
        session
            .publish(plan_b, &stopped)
            .err()
            .ok_or("stale plan accepted")?
            .code(),
    ...
    assert_eq!(
        active
            .prepare_replacement(stale, &stopped)
            .err()
            .ok_or("stale partition accepted")?
            .code(),
        GraphErrorCode::PartitionStale
    );
    ...
    assert_ne!(
        next.candidate().snapshot().generation(),
...
fn dangling_survivor_and_removed_self_endpoint_fail_closed() -> TestResult {
    ...
    assert_eq!(
        b.snapshot().edges()[0].confidence(),
        GraphConfidence::Possible
    );
    let removal = request(&b, batch("producer:a", None)?)?;
    assert_eq!(
        b.prepare_replacement(removal, &stopped)
            .err()
            .ok_or("dangling edge accepted")?
            .code(),
        GraphErrorCode::EndpointMissing
    );
    let self_reference = request(&a, edge_batch("producer:a", root, contributed.node_id())?)?;
    assert_eq!(
        a.prepare_replacement(self_reference, &stopped)
            .err()
            .ok_or("removed self endpoint accepted")?
            .code(),
...
fn cancellation_before_validation_or_publication_preserves_current() -> TestResult {
    ...
    assert_eq!(
        first
            .prepare_replacement(request.clone(), &AtomicBool::new(true))
            .err()
            .ok_or("cancelled plan accepted")?
            .code(),
    ...
    assert_eq!(
        session
            .publish(plan, &AtomicBool::new(true))
            .err()
...
fn serialized_manifest_report_and_projection_tampering_is_rejected() -> TestResult {
    ...
    for (pointer, replacement) in [
        ...
        (
            "/partitions/0/report/accepted_entities",
            serde_json::json!([]),
        ),
        ("/snapshot/nodes", serde_json::json!([])),
        (
            "/partitions/0/coverage/0/negative_authority",
        ...
        *value
        ...
        assert!(
            changed.validate(&AtomicBool::new(false)).is_err(),
            "{pointer}"
        );
    }
    Ok(())
...
fn producer_cannot_grant_negative_authority_or_exceed_assertion_budget() -> TestResult {
    ...
    assert_eq!(
        view.prepare_replacement(elevated, &AtomicBool::new(false))
            .err()
            .ok_or("authority upgrade accepted")?
            .code(),
        GraphErrorCode::PartitionInvalid
    );
    let small = GraphLimits::new(1, 10, 10, 10, 10)?;
    let base = GraphSnapshot::build(
        universe()?,
        generation()?,
        small,
        view.foundation().nodes().to_vec(),
        Vec::new(),
        coverage()?,
    )?;
    let limited =
        GraphPartitionSnapshot::new(registry()?, base, context()?, &AtomicBool::new(false))?;
    assert_eq!(
        limited
            .prepare_replacement(
                request(&limited, batch("producer:a", Some("A"))?)?,
                &AtomicBool::new(false)
            )
            .err()
            .ok_or("budget exceeded")?
            .code(),
...
fn mixed_source_context_is_rejected_before_plan_creation() -> TestResult {
    ...
    let foreign = GraphProposalBatch::build(
        ...
        universe()?,
        generation()?,
        GenerationContextId::derive(&"another-project-generation")?,
        "producer:a",
        Vec::new(),
        Vec::new(),
    )?;
    assert_eq!(
        view.prepare_replacement(request(&view, foreign)?, &AtomicBool::new(false))
            .err()
            .ok_or("mixed context accepted")?
            .code(),
