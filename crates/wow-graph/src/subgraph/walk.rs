...
pub(super) fn execute(
    ...
    for root in &query.roots {
        ...
        let node = GraphSubgraphNode {
            node: snapshot
                .node(root)
                .ok_or_else(|| invalid("subgraph root vanished"))?
                .clone(),
            depth: 0,
            discovery_edge: None,
        };
        ...
        if bytes > output_limit {
            return Err(budget(
                "subgraph output cannot hold all roots and coverage metadata",
            ));
        }
            ...
            .unwrap_or_default()
        {
            ...
            let node = if new_node {
                Some(GraphSubgraphNode {
                    node: snapshot
                        .node(next)
                        .ok_or_else(|| invalid("subgraph endpoint vanished"))?
                        .clone(),
                    depth: depth + 1,
                    discovery_edge: Some(edge.edge_id().clone()),
                })
            } else {
                None
            };
            let node_bytes = node
                .as_ref()
                .map(|node| encoded(node).map(|b| b.len() + 1))
                .transpose()?
                .unwrap_or(0);
    ...
    } else if result.coverage.iter().any(|record| {
        matches!(
            record.state(),
    ...
    } else if !result.missing_coverage.is_empty()
        || result
            .coverage
            .iter()
    ...
    result.absence_authoritative = result.no_new_evidence
        ...
        && result
            .coverage
            .iter()
            .all(GraphCoverageRecord::negative_authority);
    if encoded(&result)?.len() > output_limit {
        return Err(budget(
            "final subgraph result exceeds the canonical output budget",
        ));
    }
    checkpoint(cancelled)?;
    Ok(result)
