...
use crate::{GraphCoverageRecord, GraphCoverageState, GraphDirection, GraphEdge,
    GraphNodeId, GraphQueryState, GraphResult, GraphSnapshot};
use super::{GraphPath, GraphPathConfidence, GraphPathCursor, GraphPathQuery,
    GraphPathResult, GraphPathTruncation, GRAPH_PATH_QUERY_SCHEMA,
    budget, check_cancelled, encoded, invalid};

pub(super) fn next_node<'a>(edge: &'a GraphEdge, current: &GraphNodeId,
    direction: GraphDirection) -> Option<&'a GraphNodeId> {
    if direction != GraphDirection::Incoming && edge.from() == current { Some(edge.to()) }
    else if direction != GraphDirection::Outgoing && edge.to() == current { Some(edge.from()) }
    else { None }
}

pub(super) fn execute(query: &GraphPathQuery, snapshot: &GraphSnapshot,
    cursor: Option<&GraphPathCursor>, query_digest: Box<str>, cancelled: &AtomicBool,
) -> GraphResult<GraphPathResult> {
    let coverage = query.relations.iter().filter_map(|relation|
        snapshot.coverage_for(*relation).cloned()).collect::<Vec<_>>();
    ...
    if reserve > query.limits.max_output_bytes as usize {
        return Err(budget("path output budget cannot hold coverage and cursor metadata"));
    }
    let mut adjacency = BTreeMap::<&GraphNodeId, Vec<&GraphEdge>>::new();
    for edge in snapshot.edges() {
        check_cancelled(cancelled)?;
        if !query.confidence.admits(edge.confidence())
            || query.relations.binary_search(&edge.relation()).is_err() { continue; }
    ...
    while let Some(current) = nodes.last() {
        ...
        let position = positions.last_mut().ok_or_else(|| invalid("path stack lost its cursor"))?;
        if *position == incident.len() {
            if let Some(node) = nodes.pop() { visited.remove(&node); }
            positions.pop();
            edges.pop();
            continue;
        }
        ...
        *position += 1;
        ...
        let key = edges.iter().map(|edge| edge.edge_id().clone()).collect::<Vec<_>>();
        if !after_reached {
            ...
            if size > query.limits.max_output_bytes as usize - path_bytes {
                if paths.is_empty() { return Err(budget("one path cannot fit the output budget")); }
    ...
    if !after_reached {
        return Err(budget("path cursor boundary was not reached within the expansion budget"));
    }
    let state = if !truncations.is_empty() { GraphQueryState::Truncated }
        else if coverage.iter().any(|record| matches!(record.state(),
            GraphCoverageState::Partial | GraphCoverageState::Failed)) { GraphQueryState::Partial }
        else if coverage.len() != query.relations.len() || coverage.iter()
            .any(|record| record.state() == GraphCoverageState::NotEvaluated) {
            GraphQueryState::NotEvaluated
        } else { GraphQueryState::Complete };
    let absence_authoritative = paths.is_empty() && cursor.is_none()
    ...
    let continuation = if truncations.contains(&GraphPathTruncation::Paths)
        ...
        paths.last().map(|path| GraphPathCursor::new(&query_digest, path.key())).transpose()?
    } else { None };
    let result = GraphPathResult {
        schema: GRAPH_PATH_QUERY_SCHEMA.into(), snapshot_id: snapshot.snapshot_id().clone(),
        query_digest, state, paths, coverage, expansions, prior_truncation: cursor.is_some(),
        absence_authoritative, truncations: truncations.into_iter().collect(), continuation,
    };
    check_cancelled(cancelled)?;
