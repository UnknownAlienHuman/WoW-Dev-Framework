...
impl<'a> NavigationIndex<'a> {
    ...
    /// Convert a generated byte range to editor coordinates in the chosen encoding.
    ///
    /// The path and viewed-file digest bind the range to this immutable generation.
    /// This converts coordinates only; it does not select or interpolate source maps.
    /// Reversed ranges, mid-codepoint endpoints and the interior of CRLF reject.
    /// End-of-line and EOF are retained exactly, including zero-width ranges.
    pub fn generated_text_range(
        &self,
        generated_path: &str,
        expected_sha256: &str,
        range: Span,
        ...
        let file = self.viewed_file(generated_path, expected_sha256, cancelled)?;
        file.lines.text_range(range, encoding, cancelled)
    }
