...
pub(super) fn resolve(
    ...
    let positions: BTreeMap<_, _> = ids.iter().enumerate().map(|(i, id)| (id.as_str(), i)).collect();
    ...
    for kind in [XmlReferenceKind::Parent, XmlReferenceKind::Inherits] {
        ...
        for reference in references.iter().filter(|r| r.kind == kind) {
            checkpoint(stop)?;
            if let XmlReferenceResolution::UniqueLocalDeclaration { declaration_id } = &reference.resolution {
                edges[positions[reference.source_id.as_str()]].push(positions[declaration_id.as_str()]);
            }
        }
        ...
        for (group, nodes) in components.iter().enumerate() {
            for node in nodes { membership[*node] = Some(group); }
        }
        for (i, reference) in references.iter().enumerate().filter(|(_, r)| r.kind == kind) {
            checkpoint(stop)?;
            if let XmlReferenceResolution::UniqueLocalDeclaration { declaration_id } = &reference.resolution {
                let source = membership[positions[reference.source_id.as_str()]];
                let target = membership[positions[declaration_id.as_str()]];
                if let Some(group) = source && source == target { refs[group].push(i); }
            }
        }
        for (nodes, indices) in components.into_iter().zip(refs) {
            ...
            let reference_ids: Vec<_> = indices.iter().map(|i| references[*i].reference_id.clone()).collect();
            if reference_ids.is_empty() { return Err(super::super::invalid("XML cycle has no edges")); }
            let digest = crate::identity::canonical_digest("wow-project/xml-reference-cycle/1",
                &(kind, &declaration_ids, &reference_ids), ProjectPhase::Inventory)?;
            let cycle_id = format!("xml-cycle:{digest}");
            for i in indices { references[i].cycle_id = Some(cycle_id.clone()); }
            cycles.push(XmlReferenceCycle { cycle_id, kind, declaration_ids, reference_ids });
        }
    }
    Ok(cycles)
}
