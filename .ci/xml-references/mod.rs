...
fn retain_text(used: &mut usize, bytes: usize) -> ProjectResult<()> {
    *used = used.checked_add(bytes).ok_or_else(super::budget)?;
    if *used > MAX_RETAINED_TEXT_BYTES { return Err(super::budget()); }
    Ok(())
}
...
pub enum XmlReferenceResolution {
    UniqueLocalDeclaration { declaration_id: String },
    /// Key into the report's shared `names` table; no quadratic candidate copies.
    AmbiguousName { name_group: String },
    NotInCapturedScope,
    DynamicName,
    UnsupportedName,
    InvalidSource,
    InvalidTarget { declaration_id: String },
}
...
impl XmlReferenceReport {
    #[must_use]
    pub const fn digest(&self) -> ContentDigest<CanonicalResult> { self.digest }
    #[must_use]
    pub fn declarations(&self) -> &BTreeMap<String, XmlDeclarationSite> { &self.declarations }
    #[must_use]
    pub fn names(&self) -> &BTreeMap<String, Vec<String>> { &self.names }
    #[must_use]
    pub fn references(&self) -> &[XmlReferenceRecord] { &self.references }
    #[must_use]
    pub fn cycles(&self) -> &[XmlReferenceCycle] { &self.cycles }
    #[must_use]
    pub fn issues(&self) -> &[XmlReferenceIssue] { &self.issues }
    ...
    #[must_use]
    pub fn local_links_resolved(&self) -> bool { self.issues.is_empty() }
    #[must_use]
    pub fn unique_link_count(&self) -> usize {
        self.references.iter().filter(|r| matches!(r.resolution,
            XmlReferenceResolution::UniqueLocalDeclaration { .. })).count()
    }
}

fn issue(issues: &mut Vec<XmlReferenceIssue>, kind: XmlReferenceIssueKind,
    site: &XmlDeclarationSite, span: &XmlSourceSpan, reference_id: Option<&str>,
) -> ProjectResult<()> {
    if issues.len() >= MAX_ISSUES { return Err(super::budget()); }
    issues.push(XmlReferenceIssue {
        kind, source_id: site.occurrence_id.clone(), span: span.clone(),
        reference_id: reference_id.map(str::to_owned), cycle_id: None,
    });
    Ok(())
}
...
fn lookup(name: &str, source: &XmlDeclarationSite,
    names: &BTreeMap<String, Vec<String>>,
    sites: &BTreeMap<String, XmlDeclarationSite>,
) -> XmlReferenceResolution {
    if !source.valid_declaration { return XmlReferenceResolution::InvalidSource; }
    if name.contains(['$', '[', ']']) { return XmlReferenceResolution::DynamicName; }
    ...
    let Some(group) = names.get(name) else { return XmlReferenceResolution::NotInCapturedScope; };
    if group.len() != 1 { return XmlReferenceResolution::AmbiguousName { name_group: name.to_owned() }; }
    let target = &group[0];
    if !sites[target].valid_declaration {
        return XmlReferenceResolution::InvalidTarget { declaration_id: target.clone() };
    }
    XmlReferenceResolution::UniqueLocalDeclaration { declaration_id: target.clone() }
}
...
fn order(source: &XmlDeclarationSite, target: &XmlDeclarationSite) -> XmlReferenceOrder {
    ...
    let ([first], [second]) = (source.load_ordinals.as_slice(), target.load_ordinals.as_slice())
        else { return XmlReferenceOrder::Unrecorded; };
    if source.occurrence_id == target.occurrence_id { XmlReferenceOrder::SelfReference }
    else if second < first { XmlReferenceOrder::TargetBeforeSource }
    else { XmlReferenceOrder::TargetAfterSource }
}

fn attribute_span<'a>(element: &'a XmlElementRecord, name: &str) -> ProjectResult<&'a XmlSourceSpan> {
    element.attributes.iter().find(|a| a.qualified_name == name).map(|a| &a.value_span)
...
pub(super) fn resolve(
    ...
    for (document, index) in documents {
        for element in index.declarations() {
            checkpoint(stop)?;
            if declarations.len() >= MAX_DECLARATIONS { return Err(super::budget()); }
            let declaration = element.declaration.as_ref()
            ...
            retain_text(&mut retained_text_bytes, document.len() + element.qualified_name.len() + name_bytes * 2)?;
            let site = XmlDeclarationSite {
                document: document.clone(), content_digest: index.source_digest(),
                occurrence_id: id.clone(), element_name: element.qualified_name.clone(),
                span: element.start_tag_span.clone(), name: declaration.name.clone(),
                container_occurrence_id: element.parent_occurrence_id.clone(),
                virtual_template: declaration.virtual_template, intrinsic: declaration.intrinsic,
                valid_declaration: element.issues.is_empty(), load_ordinals: Vec::new(),
            };
            if !site.valid_declaration {
                issue(&mut issues, XmlReferenceIssueKind::InvalidDeclaration, &site, &site.span, None)?;
            }
            ...
            if offsets.insert((document.as_str(), element.start_tag_span.byte_end), id.clone()).is_some()
    ...
    for record in load_records {
        ...
        if record.kind == LoadRecordKind::XmlElement
            ...
            declarations.get_mut(id)
                .ok_or_else(|| super::invalid("XML load occurrence is missing"))?
                .load_ordinals.push(record.ordinal);
        }
    }
    ...
    for group in names.values_mut() {
        ...
        if group.len() > 1 {
            for id in group.iter() {
                ...
                issue(&mut issues, XmlReferenceIssueKind::DuplicateName, site, &site.span, None)?;
            }
        }
    }
    ...
    for index in documents.values() {
        for element in index.declarations() {
            ...
            let parent = declaration.parent_reference.iter().map(|s| (XmlReferenceKind::Parent, 0, s));
            let inherits = declaration.inherits.iter().enumerate()
                .map(|(ordinal, s)| (XmlReferenceKind::Inherits, ordinal as u64, s));
            for (kind, ordinal, name) in parent.chain(inherits) {
                checkpoint(stop)?;
                if references.len() >= MAX_REFERENCES { return Err(super::budget()); }
                retain_text(&mut retained_text_bytes, name.len() * 2)?;
                let span = attribute_span(element, match kind {
                    XmlReferenceKind::Parent => "parent", XmlReferenceKind::Inherits => "inherits",
                })?;
                ...
                let digest = crate::identity::canonical_digest(XML_REFERENCE_PROFILE,
                    &(&source.occurrence_id, kind, ordinal, name, span), ProjectPhase::Inventory)?;
                ...
                let problem = match &resolution {
                    ...
                    XmlReferenceResolution::AmbiguousName { .. } => Some(XmlReferenceIssueKind::AmbiguousReference),
                    XmlReferenceResolution::NotInCapturedScope => Some(XmlReferenceIssueKind::NotInCapturedScope),
                    XmlReferenceResolution::DynamicName => Some(XmlReferenceIssueKind::DynamicReference),
                    XmlReferenceResolution::UnsupportedName => Some(XmlReferenceIssueKind::UnsupportedReference),
                    XmlReferenceResolution::InvalidSource => Some(XmlReferenceIssueKind::InvalidSource),
                    XmlReferenceResolution::InvalidTarget { .. } => Some(XmlReferenceIssueKind::InvalidTarget),
                };
                if let Some(problem) = problem {
                    issue(&mut issues, problem, source, span, Some(&reference_id))?;
                }
                let order_problem = match load_order {
                    Some(XmlReferenceOrder::TargetAfterSource) => Some(XmlReferenceIssueKind::TargetAfterSource),
                    Some(XmlReferenceOrder::RepeatedLoad) => Some(XmlReferenceIssueKind::RepeatedLoad),
                    Some(XmlReferenceOrder::Unrecorded) => Some(XmlReferenceIssueKind::UnrecordedLoad),
                    _ => None,
                };
                ...
                if kind == XmlReferenceKind::Inherits && !seen.insert(name.as_str()) {
                    issue(&mut issues, XmlReferenceIssueKind::DuplicateInheritance, source, span, Some(&reference_id))?;
                }
                references.push(XmlReferenceRecord {
                    reference_id, source_id: source.occurrence_id.clone(), kind, ordinal,
                    name: name.clone(), attribute_span: span.clone(), resolution,
                    order: load_order, cycle_id: None,
                });
            }
        }
    }
    let cycles = cycles::resolve(&declarations, &mut references, stop)?;
    let by_reference: BTreeMap<_, _> = references.iter().map(|r| (r.reference_id.as_str(), r)).collect();
    for cycle in &cycles {
        checkpoint(stop)?;
        let first = by_reference.get(cycle.reference_ids[0].as_str())
        ...
        issue(&mut issues, kind, source, &first.attribute_span, Some(&first.reference_id))?;
        if let Some(last) = issues.last_mut() { last.cycle_id = Some(cycle.cycle_id.clone()); }
    }
    drop(by_reference);
    let document_indexes: BTreeMap<String, ContentDigest<CanonicalResult>> = documents.iter().map(|(path, index)| (path.clone(), index.digest())).collect();
    let scope = "captured_xml_declarations_only";
    let digest = crate::identity::canonical_digest(XML_REFERENCE_PROFILE,
        &(XML_REFERENCE_PROFILE, scope, &document_indexes, &declarations,
            &names, &references, &cycles, &issues), ProjectPhase::Inventory)?;
    checkpoint(stop)?;
    Ok(XmlReferenceReport {
        profile: XML_REFERENCE_PROFILE, scope,
        document_indexes, declarations, names, references, cycles, issues, digest,
    })
}
