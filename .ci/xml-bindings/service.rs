...
pub(super) fn append_findings(
    ...
    for binding in report.bindings() {
        ...
        if !selected.contains(&binding.document) || binding.state == XmlLuaBindingState::UniqueAnalyzerDeclaration {
            continue;
        }
        let original = plan.document_text(&binding.document)
        ...
        let end = usize::try_from(binding.attribute_span.byte_end)
            .map_err(|_| super::owner_error("XML Lua binding coordinate overflow"))?;
        if original.get(start..end).is_none() { return Err(super::owner_error("XML Lua binding coordinate mismatch")); }
        let location = ExactSourceLocation::new(binding.document.as_str(), binding.content_digest.to_string(),
            binding.attribute_span.byte_start, binding.attribute_span.byte_end)?;
        let id = crate::identity::canonical_digest("service-xml-lua-binding:sha256:", &(report.analysis_id(), binding))?;
        findings.push(GenericFinding::new(id, "project.xml.lua_bindings", binding.state.code(),
